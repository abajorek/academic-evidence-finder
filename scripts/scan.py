...
def add_edgar_args(parser):
    parser.add_argument("--edgar", action="store_true", help="Enable Edgar prank mode")
    parser.add_argument("--edgar-interval", type=float, default=1.0, help="Seconds between splash prints")
    parser.add_argument("--edgar-prank", action="store_true", help="Trigger Flagrant System Error loop after run")
    parser.add_argument("--edgar-splash", action="store_true", help="Show ASCII splash art before scanning")
    return parser
...
def main():
    ...
    _edgar_cm = None
    try:
        if getattr(args, "edgar_splash", False):
            print("🦆  QUACK! Welcome to Edgar mode.")
        if getattr(args, "edgar", False):
            from scripts import edgar
            _edgar_cm = edgar.edgar_context(
                interval=getattr(args, "edgar_interval", 0.5),
                show_splash=getattr(args, "edgar_splash", False)
            )
            _edgar_cm.__enter__()
    except Exception as e:
        print("Failed to start Edgar prank:", e)
    ...
    rows = []
    PROPRIETARY_HINTS = {
        ".sib", ".musx", ".mus", ".ftmx", ".ftm", ".3dj", ".3dz", ".3da", ".prod"
    }
    ...
    for path in files_to_scan:
        ...
        # Treat known proprietary extensions as filename text when extractor returns empty
        if not text_content and ext in PROPRIETARY_HINTS:
            text_content = base.lower()
        ...
        for cat, subs in compiled.items():
            for sub, pats in subs.items():
                score, hits = score_text(text_content, pats, per_hit, cap)
                if score > 0:
                    hit_this_file = True
                    ...
                    if isinstance(bonus, dict):
                        for k, pts in bonus.items():
                            try:
                                if k and re.search(re.escape(k), text_content, re.IGNORECASE):
                                    bsum += float(pts)
                            except Exception:
                                pass
    ...
    if ics_paths:
        ...
        for item_path in ics_paths:
            for item in iter_ics([item_path]):
                ...
                for cat, subs in compiled.items():
                    for sub, pats in subs.items():
                        score, hits = score_text(text_i, pats, per_hit, cap)
                        if score > 0:
                            rows.append({
                                "source": "calendar",
                                "path": "ics",
    ...
    if mbox_paths:
        ...
        for item_path in mbox_paths:
            for item in iter_mbox([item_path]):
                ...
                for cat, subs in compiled.items():
                    for sub, pats in subs.items():
                        score, hits = score_text(text_m, pats, per_hit, cap)
                        if score > 0:
                            rows.append({
                                "source": "email",
                                "path": "mbox",
    ...
    if _edgar_cm is not None:
        _edgar_cm.__exit__(None,None,None)
    if getattr(args,"edgar_prank",False):
        try:
            from scripts.edgar import flagrant_system_error_loop
            flagrant_system_error_loop()
